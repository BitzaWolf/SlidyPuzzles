...
/**
**	this is a FilterInputStream that applies a SoundFilter to the underlying input stream
**/
public class FilteredSoundStream extends FilterInputStream
{
	...
	**/
	...
	/**
	**	Overrieds the FilterInputStream method to apply this filter whenever bytes are read
	**/
	public int read(byte[] samples, int offset, int length) throws IOException
	{
		...
		// if there are no remaining bytes in the sound stream, check if the filter has any remaining bytes
		if (remainingSize == REMAINING_SIZE_UNKNOWN)
		{
			remainingSize = soundFilter.getRemainingSize();
			// round down to nearest multiple of 4, which is a typical frame size
